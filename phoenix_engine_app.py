...
============================== 45 passed in 12.34s ===============================
    ...
    try:
        conn = sqlite3.connect(DB_FILE); actions_df = pd.read_sql_query("SELECT timestamp, action, user FROM audit_log", conn); actions_df['timestamp'] = pd.to_datetime(actions_df['timestamp']); feedback_df = pd.read_sql_query("SELECT rating FROM user_feedback", conn); avg_rating = feedback_df['rating'].mean() if not feedback_df.empty else "N/A"; db_status = "Connected"; db_status_color = "normal"; conn.close()
    except Exception as e:
    ...
    kpi1, kpi2, kpi3 = st.columns(3); kpi1.metric("Total Logged Actions (30d)", len(actions_df)); kpi2.metric("Average User Feedback", f"{avg_rating:.2f} / 5" if isinstance(avg_rating, float) else avg_rating); kpi3.metric("Backend Database Status", db_status, delta_color=db_status_color); st.markdown("---")
