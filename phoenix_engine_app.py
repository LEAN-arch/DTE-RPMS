...
def generate_summary_pptx(study_id, kpi_data):
    prs = Presentation(); slide_layout = prs.slide_layouts[1]; slide = prs.slides.add_slide(slide_layout); title = slide.shapes.title; title.text = f"Executive QC Summary: Study {study_id}"; content_shape = slide.shapes.placeholders[1]; tf = content_shape.text_frame; tf.clear(); p = tf.add_paragraph(); p.text = "Key Data Quality & Integrity Metrics"; p.font.bold = True
    for k, v in kpi_data.items(): p = tf.add_paragraph(); p.text = f"{k}: {v}"; p.level = 1
    tf.add_paragraph().text = "\nThis summary was auto-generated by the Phoenix Engine 4.0."; ppt_io = io.BytesIO(); prs.save(ppt_io); ppt_io.seek(0); return ppt_io
@st.cache_data(ttl=900)
def generate_preclinical_data(study_id, n_samples=1000):
    np.random.seed(hash(study_id) % (2**32 - 1)); operators=['J.Doe', 'S.Chen', 'M.Gupta', 'R.Valdez']; instruments={'PK':['Agilent-6470', 'Sciex-7500'],'Tox':['Tecan-Spark','BMG-Pherastar'],'CF':['Ussing-Chamber-A','Ussing-Chamber-B']}; assay_type = study_id.split('-')[1]
    ...
    data={'SampleID':[f"{study_id}-S{i:04d}" for i in range(n_samples)],'Timestamp':[datetime.now()-timedelta(days=np.random.uniform(1,90),hours=h) for h in range(n_samples)],'OperatorID':np.random.choice(operators,n_samples,p=[0.4,0.3,0.2,0.1]),'InstrumentID':np.random.choice(instruments.get(assay_type,['Generic-Inst-01']),n_samples),'ReagentLot':np.random.choice([f"LOT-2024-{'A'*(i+1)}" for i in range(4)],n_samples,p=[0.7,0.15,0.1,0.05]),'Dose_uM':doses,'Response':base_response+np.random.normal(0,3,n_samples),'CellViability':np.random.normal(95,4,n_samples).clip(70,100)}; df=pd.DataFrame(data)
    df.loc[df['OperatorID']=='R.Valdez','Response']*=1.15; df.loc[df['ReagentLot']=='LOT-2024-AAAA','Response']*=0.85; df.loc[df['ReagentLot']=='LOT-2024-AAAA','CellViability']-=10; late_samples=df.sort_values('Timestamp').tail(50).index; df.loc[late_samples,'Response']+=np.linspace(0,15,50); df['QC_Flag']=0; df.loc[df[df['OperatorID']=='R.Valdez'].sample(frac=0.8).index,'QC_Flag']=1; df.loc[df[df['ReagentLot']=='LOT-2024-AAAA'].sample(frac=0.8).index,'QC_Flag']=1; late_sample_indices_to_flag=np.random.choice(late_samples,size=int(len(late_samples)*0.8),replace=False); df.loc[late_sample_indices_to_flag,'QC_Flag']=1; return df.sort_values('Dose_uM').reset_index(drop=True)
@st.cache_data
def load_data_with_dask(filepath):
    df = generate_preclinical_data("VX-LARGE-SCALE-01", n_samples=50000); ddf = dd.from_pandas(df, npartitions=4); progress_bar = st.progress(0, text="Processing large dataset with Dask..."); result = ddf.groupby('ReagentLot').Response.mean().compute(); progress_bar.progress(100, text="Processing Complete!"); return result
@st.cache_data(ttl=900)
def generate_global_kpis():
    sites = ["Boston, USA", "San Diego, USA", "Oxford, UK"]; data = []
    for site in sites: data.append({'Site': site, 'lon': [-71.0589, -117.1611, -1.2577][sites.index(site)], 'lat': [42.3601, 32.7157, 51.7520][sites.index(site)],'Studies_Active': np.random.randint(5, 15), 'Data_Integrity': f"{np.random.uniform(99.5, 99.9):.2f}%",'Automation_Coverage': np.random.randint(85, 98), 'Critical_Flags': np.random.randint(0, 5),'Mfg_OEE': np.random.randint(75, 92), 'Cpk_Avg': f"{np.random.uniform(1.3, 1.8):.2f}"}); return pd.DataFrame(data)
@st.cache_data(ttl=900)
def generate_process_data(process_name="TRIKAFTA_API_Purity"):
    np.random.seed(hash(process_name) % (2**32 - 1)); data = {'BatchID': [f'MFG-24-{i:03d}' for i in range(1, 101)], 'Timestamp': pd.to_datetime(pd.date_range(end=datetime.now(), periods=100)), 'Value': np.random.normal(99.5, 0.2, 100)}; df = pd.DataFrame(data); df.loc[75:, 'Value'] += 0.35; return df
def execute_r_spc_chart(data_df, title):
    if not RPY2_INSTALLED: return None
    try:
        qcc = importr('qcc'); grdevices = importr('grDevices'); r_df = pandas2ri.py2rpy(data_df); ro.globalenv['r_df'] = r_df; ro.globalenv['chart_title'] = title
        ...
        logger.error(f"Rpy2 execution failed: {e}"); return None
with st.sidebar:
    st.image("https://d1io3yog0oux5.cloudfront.net/_3f03b2222d6fdd47976375a7337f7a69/vertexpharmaceuticals/db/387/2237/logo.png", width=220); st.title(CONFIG['ui_settings']['dashboard_title']); st.markdown("##### GxP Hyper-Automation Platform"); st.markdown("---")
...
if page == "🌎 **Global Command Center**":
    st.header("🌎 Global RPMS Operations Command Center"); st.markdown("Real-time, holistic view of data operations...")
    ...
    st.markdown("---"); map_col,alerts_col=st.columns([2,1])
    with map_col: st.subheader("Global Site Status & Health"); global_kpis = generate_global_kpis(); fig = go.Figure(data=go.Scattergeo(lon=global_kpis['lon'], lat=global_kpis['lat'],text=global_kpis.apply(lambda row: f"<b>{row['Site']}</b><br>Integrity: {row['Data_Integrity']}<br>Automation: {row['Automation_Coverage']}%<br>OEE: {row['Mfg_OEE']}%<br>Flags: {row['Critical_Flags']}", axis=1),mode='markers',marker=dict(color=global_kpis['Critical_Flags'], colorscale=[[0, '#00AEEF'], [1, '#FF4136']], size=global_kpis['Automation_Coverage'] / 4))); fig.update_layout(geo=dict(scope='world')); st.plotly_chart(fig, use_container_width=True)
    with alerts_col: st.subheader("Priority Action Items"); st.error("🔴 **CRITICAL:** [TRIKAFTA MFG] Cpk for API Purity dropped..."); st.warning("🟠 **WARNING:** [CASGEVY QC] Reagent Lot LOT-2024-AAAA..."); st.info("🔵 **INFO:** [VX-522 Dev] New dose-response data..."); st.info("🔵 **INFO:** [FDA-REQ-003] Dossier for VTX-809-PK-01...")
elif page == "🔬 **Assay Dev & Dose-Response**":
    st.header("🔬 Assay Development & 3D Dose-Response Modeling"); st.markdown("Analyze in-vitro assay data, fit dose-response curves...")
    ...
    with c2: st.write("");st.write("");
        if st.button("📦 Export Raw Data", help="..."): csv=df.to_csv(index=False).encode('utf-8'); st.download_button(label="⬇️ Download CSV",data=csv,file_name=f'{selected_study}_raw_data.csv',mime='text/csv'); log_action("engineer.principal@vertex.com", "EXPORT_RAW_DATA", selected_study);st.success("Raw data prepared...")
    tab1,tab2,tab3=st.tabs(["📈 **2D Dose-Response Curve (IC50)**","✨ **3D Response Surface**","📦 **Batch Box Plots**"])
    with tab1: st.subheader(f"Dose-Response Curve for {selected_study}"); fig=px.scatter(df,x="Dose_uM",y="Response",log_x=True,); max_resp=df['Response'].max(); ic50_approx=df.iloc[(df['Response']-max_resp/2).abs().argsort()[:1]]['Dose_uM'].values[0]; fig.add_vline(x=ic50_approx,line_dash="dash",annotation_text=f"IC50 ≈ {ic50_approx:.2f} µM"); fig.add_hline(y=max_resp/2,line_dash="dash"); st.plotly_chart(fig, use_container_width=True)
    with tab2: st.subheader("3D Response Surface..."); df['Time_h']=df.index/len(df)*48; fig3d=px.scatter_3d(df.sample(500),x='Dose_uM',y='Time_h',z='Response',color='CellViability',log_x=True,); st.plotly_chart(fig3d, use_container_width=True)
    with tab3: st.subheader("Response Distribution by Reagent Lot"); fig_box=px.box(df,x='ReagentLot',y='Response',color='ReagentLot',); st.plotly_chart(fig_box, use_container_width=True)
    ...
    if sample_id:
        ...
        with tab1:
            st.subheader("On-Target Editing Efficiency Distribution"); c1, c2 = st.columns(2)
            with c1:
                fig_hist = px.histogram(editing_df, x='EditingEfficiency', nbins=10, title='Efficiency Across Batches'); fig_hist.add_vline(x=90, line_dash="dash", annotation_text="Release Spec (>90%)"); st.plotly_chart(fig_hist, use_container_width=True)
            with c2:
                outcomes = {'Outcome': ['Desired Edit', 'Indel (Small)', 'No Edit'], 'Percentage': [93.5, 5.2, 1.3]}; fig_pie = px.pie(outcomes, values='Percentage', names='Outcome', title='Aggregate Editing Outcomes', hole=0.4); st.plotly_chart(fig_pie, use_container_width=True)
        with tab2:
            st.subheader("Off-Target Site Scoring & Prioritization"); st.markdown("This plot scores potential off-target sites based on experimental GUIDE-seq scores and observed indel frequencies.")
            fig_ot = px.scatter(off_target_df, x='GUIDESeqScore', y='IndelFreq', size='MismatchCount', color='IndelFreq', title='Off-Target Site Risk Assessment', hover_name='Site', log_x=True, color_continuous_scale='Reds'); st.plotly_chart(fig_ot, use_container_width=True)
        with tab3:
            st.subheader("Initial Whole-Genome CNV Sanity Check"); st.info("This scan serves as a quality control step to ensure the patient's genomic baseline is free of large-scale variations that could confound gene-editing analysis. No significant CNVs detected in this sample.")
    ...
    study_list = ["VX-CF-MOD-01", "VX-522-Tox-02", "VX-PAIN-TGT-05"]; selected_studies = st.multiselect("Select studies to compare:", study_list, default=study_list)
    if selected_studies:
        data_frames = [generate_preclinical_data(s).assign(StudyID=s) for s in selected_studies]; all_data = pd.concat(data_frames, ignore_index=True)
        tab1, tab2, tab3 = st.tabs(["📊 **ANOVA & Post-Hoc Analysis**", "🎻 **Violin & Box Plots**", "🛰️ **3D Principal Component Analysis**"])
        with tab1:
            st.subheader("Analysis of Variance (ANOVA) & Tukey's HSD"); model = ols('Response ~ C(StudyID)', data=all_data).fit(); anova_table = sm.stats.anova_lm(model, typ=2); st.dataframe(anova_table)
            p_value = anova_table['PR(>F)'].iloc[0]
            if p_value < 0.05:
                st.error(f"**Statistically Significant Difference Detected (p = {p_value:.4f}).**"); tukey = pairwise_tukeyhsd(endog=all_data['Response'], groups=all_data['StudyID'], alpha=0.05); st.subheader("Tukey's HSD Post-Hoc Test Results"); st.dataframe(pd.DataFrame(tukey._results_table.data[1:], columns=tukey._results_table.data[0]))
                fig = go.Figure()
                for i, res in enumerate(tukey.summary().tables[1].data[1:]):
                    group1, group2, _, meandiff, lower, upper, reject = res
                    fig.add_shape(type="line", x0=i, y0=float(lower), x1=i, y1=float(upper), line=dict(color="RoyalBlue", width=3)); fig.add_trace(go.Scatter(x=[i], y=[float(meandiff)], mode="markers", marker=dict(color="RoyalBlue"), name=f"{group1}-{group2}"))
                fig.update_xaxes(tickvals=list(range(len(tukey.groupsunique)*(len(tukey.groupsunique)-1)//2)), ticktext=[f"{res[0]}-{res[1]}" for res in tukey.summary().tables[1].data[1:]]); fig.add_hline(y=0, line_dash="dash", line_color="red"); fig.update_layout(title="95% Confidence Intervals for Group Mean Differences"); st.plotly_chart(fig, use_container_width=True)
            else: st.success("No statistically significant difference detected.")
        with tab2:
            st.subheader("Response Distribution by Study"); fig_violin = px.violin(all_data, x='StudyID', y='Response', color='StudyID', box=True, points="all", title="Assay Response Distribution Across Studies"); st.plotly_chart(fig_violin, use_container_width=True)
        with tab3:
            st.subheader("3D PCA for Outlier/Cluster Detection"); df_pca = all_data[['Dose_uM', 'Response', 'CellViability']].dropna(); pca = PCA(n_components=3); components = pca.fit_transform(df_pca); pca_df = pd.DataFrame(data=components, columns=['PC1', 'PC2', 'PC3']); pca_df['StudyID'] = all_data.loc[df_pca.index, 'StudyID']; total_var = pca.explained_variance_ratio_.sum() * 100
            fig_pca_3d = px.scatter_3d(pca_df, x='PC1', y='PC2', z='PC3', color='StudyID', title=f'3D PCA of Preclinical Data ({total_var:.1f}% Variance Explained)'); st.plotly_chart(fig_pca_3d, use_container_width=True)

elif page == "💡 **Automated Root Cause Analysis**":
    st.header("💡 Automated Root Cause Analysis (RCA) Engine"); st.markdown("Leverages machine learning to predict the likely cause of QC flags and provides interactive tools for investigation.")
    study_id = st.selectbox("Select Study with QC Flags:", ["VX-CF-MOD-01", "VX-522-Tox-02"], key="rca_study"); df = generate_preclinical_data(study_id); df_flagged = df[df['QC_Flag'] == 1]
    if not df_flagged.empty:
        st.warning(f"Found **{len(df_flagged)}** QC-flagged records in **{study_id}**. Initiating RCA...")
        features = ['OperatorID', 'InstrumentID', 'ReagentLot']; target = 'QC_Flag'; df_ml = df.copy()
        encoders = {col: LabelEncoder().fit(df_ml[col]) for col in features}
        for col, encoder in encoders.items(): df_ml[col] = encoder.transform(df_ml[col])
        X = df_ml[features]; y = df_ml[target]; X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42, stratify=y)
        model = RandomForestClassifier(n_estimators=100, random_state=42); model.fit(X_train, y_train)
        tab1, tab2, tab3 = st.tabs(["🎯 **Predicted Root Cause**", "🔍 **Interactive Drill-Down**", "🌊 **Live SHAP Waterfall**"])
        with tab1:
            st.subheader("Probable Root Cause Contribution"); importances_vals = model.feature_importances_; importances = pd.DataFrame({'Feature': features, 'Importance': importances_vals}).sort_values('Importance', ascending=False)
            fig_imp = px.bar(importances, x='Feature', y='Importance', title='Feature Importance for QC Failures', color='Feature', color_discrete_map={"ReagentLot": "#FF4136", "OperatorID": "#FF851B", "InstrumentID": "#FFDC00"}); st.plotly_chart(fig_imp, use_container_width=True)
            top_cause = importances.iloc[0]; st.error(f"**Top Predicted Contributor:** **{top_cause['Feature']}** is the most significant factor.")
        with tab2:
            st.subheader("Visual Investigation of Top Contributor"); top_feature = importances.iloc[0]['Feature']
            fig_drill = px.box(df, x=top_feature, y='Response', color='QC_Flag', title=f"Response Distribution by {top_feature} and QC Status", color_discrete_map={0: "#00AEEF", 1: "#FF4136"}); st.plotly_chart(fig_drill, use_container_width=True)
        with tab3:
            st.subheader("Live SHAP Waterfall Plot"); st.markdown("This plot explains a single QC flag prediction, showing how each feature contributed to the final result.")
            explainer = shap.TreeExplainer(model); shap_values_obj = explainer(X_test)
            X_test_display = X_test.copy()
            for col, encoder in encoders.items(): X_test_display[col] = encoder.inverse_transform(X_test[col])
            ...
            fig, ax = plt.subplots(); shap.plots.waterfall(shap_values_obj[instance_loc_in_test, :, 1], show=False); plt.tight_layout(); st.pyplot(fig)
    ...
    else: st.success(f"No QC flags found in study **{study_id}**.")

elif page == "🚀 **Technology Proving Ground**":
    st.header("🚀 Technology Proving Ground (PoC Environment)"); st.warning("**For Demonstration Only:** This area is for evaluating and prototyping emerging technologies. Results are not for GxP use.")
    tab_langchain, tab_dask, tab_r = st.tabs(["📄 **GenAI: LangChain Summarization**", "💨 **Scalability: Dask Processing**", "📊 **Analytics: R Integration**"])
    with tab_langchain:
        st.subheader("Proof-of-Concept: Automated Report Summarization"); st.markdown("This PoC demonstrates how **LangChain** could be used to automatically generate a human-readable summary from a structured QC report.")
        report_text = st.text_area("Paste Structured Report Data Here:", height=200, value='{"study_id": "VX-CF-MOD-01", ...}');
        if st.button("🤖 Generate Summary with LangChain PoC"):
            with st.spinner("Simulating call to LangChain API..."): time.sleep(2); st.subheader("Generated Narrative Summary:"); st.info(" **Study VX-CF-MOD-01 QC Summary:**\n\nThe quality control analysis...for **lot LOT-2024-AAAA**, which failed...the failing reagent lot has been flagged and quarantined."); log_action("engineer.principal@vertex.com", "POC_LANGCHAIN_SUMMARY")
    with tab_dask:
        st.subheader("Proof-of-Concept: Large-Scale Data Processing"); st.markdown("This PoC uses **Dask** to simulate the parallel processing of a large (50,000 row) dataset...")
        if st.button("🚀 Process Large Dataset with Dask"):
            with st.spinner("Setting up Dask cluster and processing partitions..."): dask_results = load_data_with_dask("dummy_path"); st.subheader("Dask Computation Results:"); st.write("Mean 'Response' grouped by 'ReagentLot':"); st.dataframe(dask_results); log_action("engineer.principal@vertex.com", "POC_DASK_PROCESSING")
    with tab_r:
        st.subheader("Live R Integration via `rpy2`"); st.markdown("This PoC executes a real R script to generate an SPC chart using the `qcc` library...")
        if RPY2_INSTALLED:
            st.info("✅ `rpy2` is installed. Live R integration is active.")
            if st.button("📊 Generate SPC Chart with R"):
                with st.spinner("Executing R script via rpy2..."): r_data = generate_process_data("R_Integration_Test"); chart_file = execute_r_spc_chart(r_data, "SPC Chart Generated by R")
                    if chart_file and os.path.exists(chart_file): st.image(chart_file, caption="This chart was generated live by an R script and displayed in Streamlit."); log_action("engineer.principal@vertex.com", "POC_RPY2_EXECUTION")
                    else: st.error("R script execution failed. Check application logs and ensure R and the `qcc` library are installed in the environment.")
        else: st.error("**Deployment Note:** `rpy2` is not installed in this environment... The image below is a static representation of the expected output."); st.image("https://www.r-graph-gallery.com/wp-content/uploads/2018/10/Custom-Shewhart-chart-with-ggplot2.png", caption="Example of an R-generated SPC chart.")
...
============================== 45 passed in 12.34s ===============================
    ...
    with tab_kb:
        ...
        st.markdown("""
        - **IC50/EC50:** The half maximal inhibitory/effective concentration. It represents the concentration of a drug that is required for 50% inhibition (IC50) or effect (EC50) in an in-vitro assay. It is a critical measure of a compound's potency. Lower values indicate higher potency. This is estimated in the `Assay Dev` module.
        - **ANOVA (Analysis of Variance):** A statistical test used to determine whether there are any statistically significant differences between the means of two or more independent groups. For example, it can tell us *if* there is a difference in assay response across three different study drugs, but not *which* specific drugs differ.
        - **Tukey's Honest Significant Difference (HSD):** A post-hoc test run after a significant ANOVA to perform pairwise comparisons between all groups. It identifies which specific group-to-group comparisons (e.g., Drug A vs. Drug B) are statistically different while controlling for the family-wise error rate.
        - **SHAP (SHapley Additive exPlanations):** A state-of-the-art method from game theory to explain the output of any machine learning model. For a given prediction, SHAP values quantify the contribution of each feature to pushing the prediction away from the baseline. It provides deep, auditable interpretability for our `RCA Engine`.
        - **Process Capability (Cpk):** A statistical measure of a process's ability to produce output within specification limits. It measures how centered the process is between its spec limits and how widely it is spread. A `Cpk > 1.33` is a common industry target indicating a capable and reliable manufacturing process, as monitored in the `Process Control` page.
        """)
        ...
        - **21 CFR Part 11:** The FDA's regulation defining the criteria under which electronic records and electronic signatures are considered trustworthy, reliable, and equivalent to paper records. This platform addresses its key tenets via:
            - **Audit Trails:** The persistent SQLite backend logs all GxP-relevant actions (who, what, when, why) with checksums to ensure integrity.
            - **Electronic Signatures:** Actions in the `Regulatory Hub` are tied to a unique user (simulated login) and logged as a signing event.
            - **Security & Access Control:** While simulating a single user, the architecture is designed for role-based access to different modules.
            - **Validation:** The entire system is subject to the GAMP 5 validation lifecycle documented in the `System Validation` page to prove it operates as intended.
        """)
    with tab_help:
        st.subheader("Step-by-Step Guides")
        st.markdown("""
        **How to Investigate a QC Flag:**
        1. Navigate to the **Automated Root Cause Analysis** page and select the relevant study from the dropdown.
        ...
        3. Go to the second tab, 'Interactive Drill-Down', to visually confirm the issue. A box plot will show a clear difference in the response distribution for the flagged data within the problem feature's categories.
        4. Go to the third tab, 'Live SHAP Waterfall', and use the slider to inspect individual flagged records. This will show you exactly how much each feature contributed to that specific failure prediction, providing auditable evidence.
    ...
    with tab_feedback:
        ...
        with st.form("feedback_form"):
            ...
            if st.form_submit_button("Submit Feedback"):
                conn=sqlite3.connect(DB_FILE); c=conn.cursor(); c.execute("INSERT INTO user_feedback (timestamp, page, rating, comment) VALUES (?, ?, ?, ?)",(datetime.now(), feedback_page, feedback_rating, feedback_comment)); conn.commit(); conn.close()
