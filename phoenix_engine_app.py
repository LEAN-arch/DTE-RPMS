...
if page == "🌎 **Global Command Center**":
    ...
    def generate_casgevy_qc_data(sample_id):
        ...
        off_target_data.at[7, 'Site'] = 'OT_08_CRITICAL'
        off_target_data.at[7, 'IndelFreq'] = np.random.uniform(2, 4)
    ...
    if not df_flagged.empty:
        ...
        with tab3:
            ...
            # Use original string labels for display in SHAP plot
            ...
            # Create a new SHAP explanation object for display purposes
            display_explanation = shap.Explanation(
                values=shap_values_obj.values[:, :, 1][instance_loc_in_test],
                base_values=shap_values_obj.base_values[:, 1][instance_loc_in_test],
                data=X_test_display.iloc[instance_loc_in_test],
                feature_names=X_test.columns.tolist()
            )
            
            fig, ax = plt.subplots()
            shap.plots.waterfall(display_explanation, show=False)
            plt.tight_layout()
            st.pyplot(fig)
...
============================== 45 passed in 12.34s ===============================
    ...
    if not actions_df.empty:
        ...
        with c1:
            ...
            fig = px.imshow(activity, labels=dict(x="Date", y="Hour of Day", color="Actions"),
                            title="User Activity by Hour and Day", aspect="auto")
            st.plotly_chart(fig, use_container_width=True)
        with c2:
